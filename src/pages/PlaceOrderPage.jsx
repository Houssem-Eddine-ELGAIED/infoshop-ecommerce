...
const PlaceOrderPage = () => {
  ...
  // Nous définissons la taxe à 0 ici
  const taxPrice = 0;
  ...
  const placeOrderHandler = async () => {
    try {
      // Nous passons la taxe comme étant égale à 0
      const res = await createOrder({
        cartItems,
        shippingAddress,
        paymentMethod,
        itemsPrice,
        taxPrice,  // Taxe égale à 0
        shippingPrice,
        totalPrice
      }).unwrap();
        ...
        <Col md={4}>
          <Card>
            <ListGroup variant='flush'>
              ...
              {/* Tax est maintenant égal à 0 */}
              <ListGroup.Item>
                <Row>
                  <Col>Tax:</Col>
                  <Col>{addCurrency(Number(taxPrice))}</Col>  {/* Affiche 0 */}
                </Row>
              </ListGroup.Item>
              <ListGroup.Item>
                <Row>
